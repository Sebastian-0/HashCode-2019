...
public class Solution_2_double_ended {
...
//				out.write(maxP.id + "\n");
		...
		FileWriter out = new FileWriter("output/" + file.charAt(0) + "_out2_2.txt");
		out.write(pics.size() + "\n");
		System.out.println(forward.size() + " " + backward.size() + " " + pics.size());
		Collections.reverse(backward);
		for (Integer integer : backward) {
			out.write(integer + "\n");
		}
		out.write(start.id + "\n");
		for (Integer integer : forward) {
			out.write(integer + "\n");
		}
		out.close();
